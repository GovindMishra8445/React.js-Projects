...
const Signup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    setMessage("");
    try {
      // Firebase Auth से यूजर क्रिएट करें
      ...
      // डिस्प्ले नेम सेट करें
      await updateProfile(user, {
        displayName: `${form.firstName} ${form.lastName}`,
      });

      // Firestore में यूजर डिटेल्स सेव करें
      await setDoc(doc(db, "users", user.uid), {
        ...
        userType: "VENDOR", // डिफॉल्ट, admin के लिए अलग हैंडल करें
      });
  ...
  return (
    <div
      className="min-h-screen flex mx-auto items-center justify-center bg-[#f8eaea]"
      style={{ background: "linear-gradient(135deg, #fff 0%, #f8eaea 100%)" }}
    >
      <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-lg border-t-8 border-[#88070a] relative">
        <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-[#88070a] rounded-full w-20 h-20 flex items-center justify-center shadow-lg">
          <svg
        ...
        <h2 className="text-3xl font-extrabold mb-4 text-center text-[#88070a] mt-6 tracking-tight">
          Create Your Account
        </h2>
        <form
          className="space-y-5 h-[50vh] overflow-y-auto"
        ...
        >
          <div className="flex gap-4">
            <div className="flex-1">
              <label
                htmlFor="firstName"
                className="block text-sm font-semibold text-[#88070a] mb-1"
              >
                First Name
              </label>
              <input
        ...
        <p className="mt-8 text-center text-sm text-gray-600">
          ...
          <Link
            to="/login"
            className="text-[#88070a] hover:underline font-semibold transition"
          >
            Login
          </Link>
        </p>
